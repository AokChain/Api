...
Revision ID: d9f1c13394ab
Revises: 
Create Date: 2026-05-31 11:27:12.369567
...
def upgrade() -> None:
    ...
    sa.Column('reward', sa.Numeric(precision=28, scale=8), server_default='0', nullable=False),
    sa.Column('id', sa.Uuid(), nullable=False),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    op.create_index(op.f('ix_service_outputs_unlocked'), 'service_outputs', ['unlocked'], unique=False)
    ...
    sa.Column('fee', sa.Numeric(precision=28, scale=8), server_default='0', nullable=False),
    sa.Column('id', sa.Uuid(), nullable=False),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index('ix_service_transactions_addresses', 'service_transactions', ['addresses'], unique=False, postgresql_using='gin')
...
def downgrade() -> None:
    ...
    op.drop_index('ix_service_transactions_addresses', table_name='service_transactions', postgresql_using='gin')
    ...
    op.drop_index(op.f('ix_service_outputs_unlocked'), table_name='service_outputs')
